export const randomNum = (min, max, rand = Math.random()) =>
  Math.floor(rand * (max - min + 1)) + min;

export const getCoordsFromIndex = gridSize => index => ({
  x: index % gridSize,
  y: Math.floor(index / gridSize)
});

export const getIndexFromCoords = gridSize => ({ x, y }) => y * gridSize + x;
...
export function dropItems(grid, gridSize, itemsToRemove) {
  ...
  columnsToDrop.forEach((col, colIndex) => {
    ...
    for (let y = gridSize - 1; y >= 0; y--) {
      newGrid[getIndexFromCoordsForSize({ x: colIndex, y })] =
        column.pop() || null;
    }
  });
